...
class DclpWorkflowTest < ActionController::IntegrationTest
  ...
  context "for dclp" do
    context "workflow testing" do
      setup do
        ...
        @dclp_board = FactoryGirl.create(:dclp_board, :title => "DCLP_Editorial_Board")
        @dclp_board.users << @board_user
      ...
      teardown do
        begin
          ActiveRecord::Base.connection_pool.with_connection do |conn|
            count = 0
            [ @board_user, @board_user_2, @creator_user, @end_user, @meta_board, @text_board, @translation_board, @dclp_board ].each do |entity|
              count = count + 1
  ...
  context "for dclp" do
    ...
    teardown do
      ...
      ActiveRecord::Base.connection_pool.with_connection do |conn|
        ...
        @ddb_board, @hgv_meta_board, @hgv_trans_board, @dclp_board ] ).each {|entity| entity.destroy}
      end
    end
